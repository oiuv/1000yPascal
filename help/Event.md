...
本页描述炎黄新章源码实际加载的 `Event/` 数据。云端千年神武奇章线上优化版脚本可证明该运营版本的调用方式，但参数编号不能直接套用到当前程序，也不能据此推断原版神武行为。
...
> **已确认的源码/数据矛盾：** `TRandomEventItemList.DataList` 只声明并初始化 0～2 三个列表，加载循环却访问 0～3；炎黄随包的 `犀牛猎人.txt` 又调用编号 3。编号 3 会发生越界访问，不能把“文件存在”写成安全可用。云端神武版脚本还调用 4、5；当前炎黄结构不支持这些编号，迁移时必须改表或修正程序后再验证。
