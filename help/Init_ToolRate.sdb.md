...
> **注意**：源码中原本设计了基于采集经验值的概率提升机制（经验越高，多次取最小值，偏向稀有物品），但该逻辑已被注释掉（`svClass.pas` 的 `TMineObjectClass.ChoiceMineItemPos` 中 `{for i := 0 to Level do ...}` 代码块）。当前实际逻辑为单次 `Random(10000)`，不受经验影响。
